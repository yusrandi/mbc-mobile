  ...
  Stream<PerformaState> mapEventToState(PerformaEvent event) async* {
    if (event is PerformaFetchDataEvent) {
      try {
        ...
        final data = await repository.performaFetchData();
      ...
      try {
        yield PerformaLoadingState();
        await Future.delayed(const Duration(milliseconds: 30));
        final data = await repository.performaStore(event.performa);
        if (data.responsecode == "1") {
          yield PerformaSuccessState(data.responsemsg, data.performa);
        } else {
          yield PerformaErrorState(data.responsemsg, data.performa);
        }
      } catch (e) {
